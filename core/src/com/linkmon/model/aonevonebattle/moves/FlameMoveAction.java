...
public class FlameMoveAction implements IMoveAction {
	...
	public boolean doAction(GameObject player, GameObject target, int energy, LinkmonAnimationComponent animComp, OneVMove move) {
		...
		if(running) {
			...
			if(throwObject.getX() == (int)(target.getX()+(target.getWidth()/2)) && throwObject.getY() == (int)(target.getY()) && timer.stopped) {
				timer.start(); // Start timer once move hits target (gives the particles a little time to run)
			}
			else if(timer.ended)
					player.getWorld().geteManager().notify(new ModelEvent(ModelEvents.ADD_PARTICLE_EFFECT, ((OneVOneBattleComponent)player.getExtraComponents()).getCurrentMove().getParticleId(), throwObject.getX(), throwObject.getY()));
	...
	private void createParticleAttack(GameObject player, GameObject target) {
		...
		throwObject.setPosition(player.getX()+player.getHalfWidth()+(player.getHalfWidth()*player.direction), player.getY());
...
//		((MoveRenderingComponent)throwObject.getRenderer()).setSprite(throwObject);
		...
		((OneVBattlePhysicsComponent)throwObject.getPhysicsComponent()).dashTo((int)(target.getX()+(target.getWidth()/2)), (int)(target.getY()), throwObject, 20);
		
		//((MoveRenderingComponent)throwObject.getRenderer()).setParticleEffect(((OneVOneBattleComponent)player.getExtraComponents()).getCurrentMove().getId(), throwObject.getX()+throwObject.getWidth()/2, throwObject.getY()+throwObject.getHeight()/2);
	}
}
